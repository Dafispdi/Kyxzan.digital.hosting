...
<body class="bg-gray-50 min-h-screen flex flex-col"><audio autoplay="True" hidden="True" id="bgm" loop="True"><source src="https://e.top4top.io/m_342392efg0.mp3" type="audio/mpeg"/></audio>
<div class="fixed inset-0 bg-black bg-opacity-80 z-50 flex items-center justify-center" id="loginOverlay">
<div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm">
<h2 class="text-2xl font-semibold text-center text-indigo-600 mb-4">Login</h2>
<input class="w-full mb-3 px-4 py-2 border rounded focus:outline-none" id="loginUser" placeholder="Username" type="text"/>
<input class="w-full mb-3 px-4 py-2 border rounded focus:outline-none" id="loginPass" placeholder="Password" type="password"/>
<button class="w-full bg-indigo-600 text-white py-2 rounded hover:bg-indigo-700" onclick="doLogin()">Login</button>
<p class="text-red-600 text-center mt-3 hidden" id="loginError">Login gagal. Username atau password salah.</p>
</div>
</div>
...
<div>
<label class="block text-gray-700 font-medium mb-2" for="plan">
       Select Plan
      </label>
...
<div class="mt-8 p-4 bg-green-50 border border-green-300 rounded-md hidden whitespace-pre-wrap font-mono text-sm" id="result">
</div>
<div class="mt-8 p-4 bg-red-50 border border-red-300 rounded-md hidden text-red-700 font-semibold" id="error">
</div>
</section>
...
<section class="max-w-3xl mx-auto mt-16 bg-white rounded-lg shadow p-6" id="about">
<h2 class="text-3xl font-semibold text-indigo-700 mb-4 text-center">
     About This Service
    </h2>
...
<div class="container mx-auto px-4 text-center text-gray-600 text-sm">
    © 2025 KyxzanTzy Official. All rights reserved.
   </div>
</footer>
<script>
   // Mobile menu toggle
    ...
    // Global settings as per user request
    const globalSettings = {
      eggV2: "15", // Egg ID
      nestidV2: "5", // Nest ID
      locV2: "1", // Location ID
      domainV2: "https://kyxzanler.cjdw.tech",
      apikeyV2: "ptla_co0AqzMKBwGdqLCQpmGKRlXt6zMoGzuebsB6h20HBV9",
      capikeyV2: "ptlc_nMPnl13wHDUjp4Mo768ojyGK8cb45zOIFe07Q8IFX3s"
    };
    ...
    document.getElementById('panelForm').addEventListener('submit', async (e) => {
      ...
      // Set global.panel to username
      const panel = usernameInput.toLowerCase();

      // Get specs for selected plan
      ...
      const name = capitalizeUsername(username) + " Server";
      ...
      try {
        ...
        const eggResponse = await fetch(
          globalSettings.domainV2 + `/api/application/nests/${globalSettings.nestidV2}/eggs/${globalSettings.eggV2}`,
          {
            method: "GET",
            headers: {
              Accept: "application/json",
        ...
        if (eggData.errors) {
          throw new Error(JSON.stringify(eggData.errors[0], null, 2));
        }
        const startup_cmd = eggData.attributes.startup;

        // Create server
        const serverResponse = await fetch(globalSettings.domainV2 + "/api/application/servers", {
          method: "POST",
          headers: {
            Accept: "application/json",
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            description: desc,
            user: usr_id,
            egg: parseInt(globalSettings.eggV2),
            ...
            deploy: {
              locations: [parseInt(globalSettings.locV2)],
              dedicated_ip: false,
              port_range: [],
            },
          }),
        ...
        if (serverData.errors) {
          throw new Error(JSON.stringify(serverData.errors[0], null, 2));
        }
        ...
        function formatSpec(value) {
          ...
          if (value.length > 4) return value.slice(0, 2) + "GB";
          return value.charAt(0) + "GB";
        }
...
- Node: 2 0.0000001Ms
🌍 Panel Host: ${globalSettings.domainV2}
